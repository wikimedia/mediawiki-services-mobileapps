...
describe('preview-html', function() {
    ...
    it('200 For a page that does exist', () => {
        const uri = `${server.config.uri}en.wikipedia.org/v1/page/preview-html/Barack Obama`;
        return preq.get({ uri })
            .then((res) => {
    ...
    it('404 For a page that doesn\'t exist', () => {
        const uri = `${server.config.uri}en.wikipedia.org/v1/page/preview-html/ashsahahash`;
        return preq.get({ uri })
            .catch((res) => {
    ...
    it('204 for pages that are not wikitext', () => {
        ...
        const uri = `${server.config.uri}meta.wikimedia.org/v1/page/preview-html/${title}`;
        return preq.get({ uri })
            .then((res) => {
    ...
    it('204 for pages outside content namespace', () => {
        ...
        const uri = `${server.config.uri}en.wikipedia.org/v1/page/preview-html/${title}`;
        return preq.get({ uri })
            .then((res) => {
    ...
    it('200/302 for pages that are redirects', () => {
        const uri = `${server.config.uri}en.wikipedia.org/v1/page/preview-html/Barack`;
        return preq.get({ uri })
            .then((res) => {

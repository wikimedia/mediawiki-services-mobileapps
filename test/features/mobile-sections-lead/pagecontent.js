...
describe('mobile-sections-lead', function() {
	...
	it('Barack Obama infobox is part of the html', () => {
		...
		return preq.get({ uri })
			.then((res) => {
				assert.ok(res.body.sections[0].text.includes('"infobox'),
	...
	it('Enwiki hatnotes are promoted to the lead object', () => {
		...
		return preq.get({ uri })
			.then((res) => {
				...
				assert.ok(res.body.sections[0].text.includes('See also:'),
	...
	it('Enwiki Multiple page issues are promoted to lead', () => {
		...
		return preq.get({ uri })
			.then((res) => {
				...
				assert.ok(res.body.sections[0].text.includes('ambox-multiple_issues'),
	...
	it('Enwiki Pages with single issue have issue promoted to lead', () => {
		...
		return preq.get({ uri })
			.then((res) => {
				...
				assert.ok(!res.body.sections[0].text.includes('"ambox'),

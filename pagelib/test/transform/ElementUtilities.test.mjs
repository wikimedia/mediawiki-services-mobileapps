...
describe( 'ElementUtilities', () => {
	...
	describe( '.closestInlineStyle()', () => {
		describe( 'present', () => {
			it( 'inclusive', () => {
				const doc = domino.createDocument( '<br style="top: 0;">' );
				const element = doc.querySelector( 'br' );
			...
			it( 'parent', () => {
				const doc = domino.createDocument( '<div style="top: 0;"><br></div>' );
				const element = doc.querySelector( 'br' );
			...
			it( 'grandparent', () => {
				const doc = domino.createDocument( '<div style="top: 0;"><p><br></p></div>' );
				const element = doc.querySelector( 'br' );
				assert.ok( elementUtilities.closestInlineStyle( element, 'top' ).tagName === 'DIV' );
			} );

			it( 'grandparent with sought value', () => {
				const doc = domino.createDocument( '<div style="top: 55;"><p><br></p></div>' );
				const element = doc.querySelector( 'br' );
		...
		describe( 'absent', () => {
			it( 'defined', () => {
				const doc = domino.createDocument( '<div style="bottom: 0;"><p><br></p></div>' );
				const element = doc.querySelector( 'br' );
				assert.ok( !elementUtilities.closestInlineStyle( element, 'top' ) );
			} );
	...
	describe( '.copyAttributesToDataAttributes()', function Test() {
		beforeEach( () => {
			...
			this.source = doc.querySelector( '#source' );
			this.destination = doc.querySelector( '#destination' );

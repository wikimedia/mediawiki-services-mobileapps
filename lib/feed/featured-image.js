...
function requestPictureOfTheDay( app, date ) {
	...
	return api.mwApiGet( app, COMMONS_URL, {
		...
		iiextmetadatafilter: 'ImageDescription|Artist|LicenseShortName|License|LicenseUrl|Credit',
...
function buildPayload( page, lang, req ) {
	...
	payload.credit = ext && ext.Credit && ext.Credit.value;
	payload.license = {
	...
	payload.artist = ext && ext.Artist && media.getStructuredArtistInfo( req, ext.Artist.value );
	payload.file_page = imageinfo.descriptionurl;
...
function promise( app, req, lang ) {
	...
	return requestPictureOfTheDay( app, dateUtil.getRequestedDate( req ), aggregated )
		.then( ( response ) => {
			...
			ret = {
				payload: buildPayload( page, lang, req ),
				meta: buildMeta( page )
			};

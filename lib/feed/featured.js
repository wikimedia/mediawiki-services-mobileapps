...
const supportedLangs = [ "bg", "bn", "bs", "cs", "de", "el", "en", "fa", "fr",
    "he", "hu", "ja", "la", "no", "sco", "ur", "vi" ];
...
const promise = (app, req) => {
    ...
    return BBPromise.props({
        featured: requestFeaturedFeedSpecialItem(domain, requestedDate),
        siteinfo: mwapi.getSiteInfo(app, req)
    }).then((response) => {
        api.checkResponseStatus(req, response.featured);
        pageTitle = findPageTitle(response.featured.body);
        if (!pageTitle) {
            return BBPromise.resolve({});
        }
        const dbTitle = mwapi.getDbTitle(pageTitle, response.siteinfo);
        return {
            payload: {

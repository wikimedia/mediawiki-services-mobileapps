...
const supportedLangs = [ "bg", "cs", "de", "el", "en", "fa", "fr", "he", "hu", "ja", "la", "no",
    "ur", "vi" ];


const isSupported = (domain) => {
    return supportedLangs.indexOf(mUtil.getLanguageFromDomain(domain)) > -1
 ...
 */
const requestFeaturedFeedSpecialItem = (domain, date) => {
    let lang = mUtil.getLanguageFromDomain(domain);
    if (lang === 'no') {
        lang = 'nb';
    }
...
const findPageTitle = (html) => {
    ...
    let scope = document.querySelector('div#mw-content-text > p');
    if (!scope) {
        scope = document.querySelector('div#mw-content-text');
        if (!scope) {
            return;
        }
    }
...
const promise = (app, req) => {
    ...
    return requestFeaturedFeedSpecialItem(domain, requestedDate)
    .then((response) => {
        api.checkResponseStatus(req, response);
        pageTitle = findPageTitle(response.body);
        if (!pageTitle) {
            return BBPromise.resolve({});
        }
        ...
        return {
            ...
            meta: { etag: createSha1(res.dbTitle) }
        };
    }).catch((err) => {
        if (aggregated) {
            return BBPromise.resolve({});

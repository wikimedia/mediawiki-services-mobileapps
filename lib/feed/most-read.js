...
function promise(app, req) {
    ...
    return BBPromise.props({
        pageviews: getTopPageviews(app, req, mUtil.removeTLD(req.params.domain), rspDate),
        ...
        const mainPage = response.siteinfo.general && response.siteinfo.general.mainpage;
        const mainPageTitle = Title.newFromText(mainPage, response.siteinfo);

        const combinedResults = response.pageviews.combined && response.pageviews.combined.body;
        const desktopResults = response.pageviews.desktop && response.pageviews.desktop.body;
        ...
        const titles = filterSpecial(filterBots(combinedSlice, desktopSlice).map((entry) => {
            const ns = Title.newFromText(entry.article, response.siteinfo).getNamespace()._id;
        ...
        return BBPromise.all(result.map(article => BBPromise.props(article))).then((response) => {
            return {
                payload: {
                    date: resultsDate,
                    articles: response

...
function promise(app, req) {
    ...
    .then(function (response) {
        ...
        const COMPARATOR_TITLES = 2 * QUERY_TITLES;
        ...
        const combinedItems = combinedResults && combinedResults.items && combinedResults.items[0];
        const desktopItems = desktopResults && desktopResults.items && desktopResults.items[0];
        const combinedMostRead = combinedItems && combinedItems.articles && combinedItems.articles.slice(0, QUERY_TITLES);
        const desktopMostRead = desktopItems && desktopItems.articles && desktopItems.articles.slice(0, COMPARATOR_TITLES);
        ...
        const results = goodTitles.map(function(entry) {
            return Object.assign(entry, {
                $merge: [ mUtil.getRbPageSummaryUrl(app.restbase_tpl, req.params.domain, entry.article) ],
                article: undefined,

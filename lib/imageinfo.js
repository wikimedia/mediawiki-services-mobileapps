...
function makeResults(ids, wbentities, meta, siteinfo, preferredLang, desc = null) {
    ...
    return meta.filter(item => item.imageinfo).map((item) => {
        ...
        const wb_entity_id = idLookup && idLookup[item.title] && `M${idLookup[item.title].pageid}`;
...
function requestPictureOfTheDay(app, req, date, siteinfo) {
    ...
    return api.mwApiGet(app, COMMONS_URL, query, req.headers).then((response) => {
        ...
        const structuredQuery = Object.assign(strucuredCaptionsQuery, { titles: page.title });

        return P.join(
            parsoid.getParsoidHtml(app, parsoidReq),
            api.mwApiGet(app, COMMONS_URL, structuredQuery, req.headers),
            (parsoid, structured) => {
                const description = getPotdDescriptionValue(req, parsoid.body);
...
function getMetadataFromApi(app, req, titles, siteinfo) {
    ...
    return P.join(
        api.mwApiGetBatched(app, COMMONS_URL, { action: 'query' }, titles.slice(0), req.headers),
        api.mwApiGetBatched(app, COMMONS_URL, strucuredCaptionsQuery, titles.slice(0), req.headers),
        api.mwApiGetBatched(app, req.params.domain, metaDataQuery, titles.slice(0), req.headers),

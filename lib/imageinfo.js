...
function structureCaptions(wbentity) {
	...
	if (captions) {
		Object.keys(captions).forEach(index => {
...
function makeResults(ids, wbentities, meta, siteinfo, preferredLang, desc = null) {
	...
	return meta.pages.filter(item => item.imageinfo).map((item) => {
		...
		const metadata = imageInfo.metadata && metadataToObject(imageInfo.metadata);
		...
		return {
			...
			duration: metadata && (metadata.length || metadata.playtime_seconds) || undefined,
 ...
 * @param  {Object} siteinfo [description]

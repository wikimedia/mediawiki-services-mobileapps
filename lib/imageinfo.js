 ...
 */
function makeResults(commons, local, siteinfo, preferredLang, desc = null) {
    function makeResult(item, addEntityId) {
        ...
        return {
            ...
            wb_entity_id: addEntityId && item.pageid && `M${item.pageid}`
        };
    }
    return commons.map(i => makeResult(i, true)).concat(local ? local.map(i => makeResult(i)) : [])
...
function requestPictureOfTheDay(app, req, date, siteinfo) {
    ...
    return api.mwApiGet(app, COMMONS_URL, query).then((response) => {
        ...
        const potd = _.values(makeResults([page], undefined, siteinfo, lang, description))[0];
 ...
 */
function getMetadataFromApi(app, req, commonsTitles, localTitles, siteinfo) {
    ...
    return P.join(
        api.mwApiGetBatched(app, 'commons.wikimedia.org', metaDataQuery, commonsTitles),
        api.mwApiGetBatched(app, req.params.domain, Object.assign(metaDataQuery,
            { localonly: '' }), localTitles),
        (commons, local) => makeResults(commons, local, siteinfo, preferredLang));
}

...
function requestPictureOfTheDay(req, date, siteinfo) {
    ...
    return api.mwApiGet(req, query, COMMONS_URL).then((response) => {
        ...
        return P.join(
            ...
            api.mwApiGet(req, structuredQuery, COMMONS_URL),
            (parsoid, structured) => {
...
function getMetadataFromApi(req, titles, siteinfo) {
    ...
    return P.join(
        api.mwApiGetBatched(req, { action: 'query' }, titles.slice(0), COMMONS_URL),
        api.mwApiGetBatched(req, strucuredCaptionsQuery, titles.slice(0), COMMONS_URL),
        api.mwApiGetBatched(req, metaDataQuery, titles.slice(0)),

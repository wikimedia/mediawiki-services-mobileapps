...
function normalizePotdResponse(potd) {
    Object.keys(potd).forEach((key) => {
        potd = potd[key];
    });
    /**
     * Make result compatible with featured image spec
     * 1) title = titles.display && remove titles
     * 2) rename original to image
     * 3) rename html to text
     */
    ...
    // 3
    potd.description.text = potd.description.html;
    delete potd.description.html;
...
function requestPictureOfTheDay(app, req, date, siteinfo) {
    ...
    return api.mwApiGet(app, COMMONS_URL, query).then((response) => {
        ...
        const potd = makeResults(req, [page], siteinfo, descriptionValue);

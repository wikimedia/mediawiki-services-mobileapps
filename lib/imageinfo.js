 ...
 * @returns {!string} Commons domain for beta or prod
 */
...
function getMetadataFromApi(req, titles, siteinfo) {
    ...
    return P.join(
        api.mwApiGetBatched(req, { action: 'query' }, titles.slice(0), commonsDomain),
        api.mwApiGetBatched(req, structuredCaptionsQuery, titles.slice(0), commonsDomain),

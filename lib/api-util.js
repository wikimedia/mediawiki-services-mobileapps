...
/**
 * A conveninence method for calling action=query that automatically handles continuation.
 * @param {!Object} app app the application object
 * @param {string} domain the domain to issue the request to
 * @param {!Object} params the request parameters expressed as a key-value object
 * @param {?Object} prev results of previous calls in case of continuation, with 'title' values
 *   as keys and the corresponding MW API page objects as values.
 */
function mwApiQuery(app, domain, params, prev = {}) {
    return mwApiGet(app, domain, Object.assign(params, { action: 'query' }))
    .then((response) => {
        const pages = response.body.query && response.body.query.pages || [];
        const result = pages.reduce((acc, cur) => {
            if (acc[cur.title]) {
                _.extend(acc[cur.title], cur);
            } else {
                acc[cur.title] = cur;
            }
            return acc;
        ...
        if (response.body.continue) {
            return mwApiQuery(app, domain, Object.assign(params, response.body.continue), result);
        } else {
...
/**
 * Gets the REST API URI for the supplied domain and optional path. If only the API base URI
 * is needed the path parameter can be omitted.
 * @param {!Object} app the application object
 * @param {!string} domain the domain to issue the request for
 * @param {?string} path the REST API path to contact without the leading slash
 * @return {!string} the URI for RESTBase endpoints for the given domain
 */
function getRestApiUri(app, domain, path = undefined) {

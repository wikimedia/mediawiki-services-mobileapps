 ...
 * @param {?Object} headers object with request headers to send
 * @return {!Promise} a promise resolving as the response object from the MW API
 */
function mwApiGet(app, domain, query, headers) {
    ...
    const request = app.mwapi_tpl.expand({
        request: {
            params: { domain },
            headers: Object.assign({ 'user-agent': app.conf.user_agent }, headers),
            query
        }
    });
 ...
 */
function mwApiGetBatched(app, domain, query, titles, headers, size = MAX_BATCH_SIZE) {
    const reqs = _batch(titles, size).map((res) => {
        return mwApiGet(app, domain, Object.assign(query, { titles: res.join('|') }), headers);
    });

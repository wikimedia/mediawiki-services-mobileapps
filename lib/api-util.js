...
/**
 * 1. Get data from mediawiki siteinfo api
 * 2. Parse and convert this data into relevant format
 *
 * @return {!Promise} a promise resolving as the response object from the MW API
 */
function getWikiLanguageData() {
    ...
    return preq.get({
        ...
        .then((res) => {
            ...
            const resLanguageVariants = res.body.query.languagevariants;
            const convertedData = {};

            for (const property in resLanguageVariants) {

                convertedData[property] = {
                    default: {
                        default: property
                    }
                };

                // Retrieve relevant codes from fallbacks and write them down into the code object
                for (const innerProperty in resLanguageVariants[property]) {
                    resLanguageVariants[property][innerProperty].fallbacks
                        .forEach((fallbackLang) => {
                            if (innerProperty.includes('-')) {
                                convertedData[property][innerProperty.split('-')[1]] = {
                                    ...convertedData[property][innerProperty.split('-')[1]],
                                    // Additional related codes
                                    default: innerProperty,
                                    [fallbackLang.includes('-') ? fallbackLang.split('-')[1] : fallbackLang]: fallbackLang,
                                };

                                // Handle 'cyrl' and 'latn' codes for 'sr' language
                                if (property === 'sr') {

                                    if (Object.prototype.hasOwnProperty.call(convertedData[property], 'ec')) {
                                    // Delete invalid property from prev iteration & replace it
                                        delete convertedData.sr.ec;
                                        convertedData.sr.cyrl = {
                                            ...convertedData[property][innerProperty.split('-')[1]],
                                            default: innerProperty,
                                            [fallbackLang.includes('-') ? fallbackLang.split('-')[1] : fallbackLang]: fallbackLang,
                                        };
                                    ...
                                    if (Object.prototype.hasOwnProperty.call(convertedData[property], 'el')) {
                                        delete convertedData.sr.el;
                                        convertedData.sr.latn = {
                                            ...convertedData[property][innerProperty.split('-')[1]],
                                            default: innerProperty,
                                            [fallbackLang.includes('-') ? fallbackLang.split('-')[1] : fallbackLang]: fallbackLang,
                                        };
                                ...
                                // Insert fallback codes into 'default' property
                                convertedData[property] = {
                                    ...convertedData[property],
                                    default: {
                                        ...convertedData[property].default,
                                        [innerProperty.split('-')[1]]: innerProperty,
                                    }
                                };
                            }
                        });

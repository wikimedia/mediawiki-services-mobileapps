...
function mwLoadResourceLoaderModules(req, query) {
	...
	return makeOutgoingRequest(request, req).then((response) => {
		// TODO: Remove workaround after preq deprecation
...
function setupApiTemplates(app) {
	...
	if (!app.conf.mwapi_req) {
		app.conf.mwapi_req = {
			...
			headers: '{{ merge(request.headers, {content-type: "application/x-www-form-urlencoded"} )}}',
	...
	// setup the Lift Wing outlink-topic-model inference endpoint
	if (!app.conf.predicted_topics_req) {
		app.conf.predicted_topics_req = {
			method: 'POST',
			uri: 'https://api.wikimedia.org/service/lw/inference/v1/models/outlink-topic-model:predict',
			headers: {
				'content-type': 'application/json'
			},
			body: '{{request.body}}'
	...
	app.predicted_topics_tpl = new Template(app.conf.predicted_topics_req);
}
...
const BETA_COMMONS_DOMAIN = 'commons.wikimedia.beta.wmcloud.org';

const PROD_WIKIDATA_DOMAIN = 'www.wikidata.org';
const BETA_WIKIDATA_DOMAIN = 'wikidata.beta.wmcloud.org';
...
function getCommonsDomain(domain) {
	return domain.endsWith('beta.wmcloud.org') ? BETA_COMMONS_DOMAIN : PROD_COMMONS_DOMAIN;
}
...
function getWikidataDomain(domain) {
	return domain.endsWith('beta.wmcloud.org') ? BETA_WIKIDATA_DOMAIN : PROD_WIKIDATA_DOMAIN;
}

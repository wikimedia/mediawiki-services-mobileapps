 ...
 *
 */
function restApiGet(app, req, domain, path, restReq) {

    restReq = restReq || {};
    path = path[0] === '/' ? path.slice(1) : path;
    const reqTemplate = req.query && req.query.useparsoid ? app.parsoid_tpl : app.restbase_tpl;

    const request = reqTemplate.expand({
        request: {
...
function setupApiTemplates(app) {
    ...
    // set up the Parsoid request template (for diff testing only)
    if (app.conf.parsoid_req) {
        app.parsoid_tpl = new Template(app.conf.parsoid_req);
    }
}

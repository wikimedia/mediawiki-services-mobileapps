...
// Checks both the string and boolean value so as to gracefully handle the deployment
// of https://gerrit.wikimedia.org/r/#/c/412998/.
// Delete this and just check for the boolean true after the rollout (March 2018).
mwapi.arePrimaryCoordinates = (coords) => {
    return mUtil.mwApiTrue(coords, 'primary');
};
...
mwapi.getPrimaryEarthCoordinates = (coords) => {
    if (Array.isArray(coords)) {
        const primary = coords.filter(c => c.globe === 'earth' && mwapi.arePrimaryCoordinates(c));
        if (primary.length) {
            return {
...
mwapi.getMetadata = ((app, req, thumbSize = mwapi.LEAD_IMAGE_XL) => {
    ...
    return BBPromise.props({
        ...
        const thumb = thumbUrl ? { url: thumbUrl.replace('https:', '') } : undefined;
        ...
        return {
            ...
            mobileHost,
            redirect: page.redirect, // needed to test that MCS isn't handling redirects internally

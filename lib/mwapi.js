...
/**
 * Set the page description on a cenral wiki (wikidata)
 *
 * @param {!Object} req
...
mwapi.setCentralDescription = (req, description) => {
    ...
    if (req.headers['content-language']
        ...
        && wikiLanguage.getAllLanguageVariantsForLanguageCode(languageCode).indexOf(req.headers['content-language']) >= 0) {
    ...
    // It is safe to bypass CSRF token server-side since the API
    // is PUT/DELETE request with JSON content-type. That means
    // a request to this API is never a simple request, forcing the
    // browser to preflight it. Since we never respond with
    // Access-Control-Allow-Credentials, and ignore the cookie auth,
    // it's safe.
    return api.mwApiGet(req, apiParams({
        action: 'query',
        meta: 'tokens',
        type: 'csrf'
    }), wikidataDomain, authHeaders)
        .then((res) => {

...
const { HTTPTitleRedirectError } = require('./core-api-compat');
 ...
 */
mwapi.getEditToken = (req, authHeaders, altDomain = null) => api.mwApiGet(req, apiParams({
	action: 'query',
	meta: 'tokens',
	type: 'csrf'
...
mwapi.setLocalShortDescription = (req, description) => {
	...
	return mwapi.getEditToken(req, authHeaders)
		.then((token) => mwapi.getPageLatestPageContent(req, req.params.title, authHeaders)
			.then((pageInfo) => {
...
mwapi.getPageFromMobileview = function(req) {
	...
	return api.mwApiGet(req, query, req.params.domain, headers).then((response) => {
		...
		if (response.body.parse.redirects.length > 0) {
			throw new HTTPTitleRedirectError(req.params, response.body.parse.redirects[0].to);
		}
...
mwapi.resolveTitleRedirect = function(req) {
	...
	return api.mwApiGet(req, {
		...
		mwapi.checkForQueryPagesInResponse(req, res);
		const responseData = res.body.query;
		if (title) {
			if (responseData.pages) {
				req.mwPageInfo = responseData.pages[0];
			}

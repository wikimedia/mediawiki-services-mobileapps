...
mwapi.queryForMetadata = (req, query, responseBuilder) => {
    ...
    return BBPromise.join(
        ...
        (siteinfo, parse, wbEntities, metadata) => {
            ...
            const description = descriptionsUtils.parseDescription(page, entities, languages);
 ...
 *
 * @param {!Object} req the request object
 * @return {!string} resolvedTitle title resolved from redirect query
 */
mwapi.resolveTitleRedirect = function(req) {
    const { title, domain } = req.params;
    return preq.get({

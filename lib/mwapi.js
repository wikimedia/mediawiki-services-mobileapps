...
mwapi.queryForMetadata = (req, query, responseBuilder) => {
    const languageCode = api.getLanguageCode(req.params.domain);
    const isLanguageCodeWithVariants = api.isLanguageCodeWithVariants(languageCode);
    let displayTitlePromise;
    ...
    (siteinfo, parse, wbge, metadata) => {
        ...
        const description = {
            source: (page && page.descriptionsource) || 'central'
        };
        let wikidataDescription;
        if (wbge && wbge.body && wbge.body.entities) {
            for (var entityID in wbge.body.entities) {
                const entity = wbge.body.entities[entityID];
                ...
                for (var i = 0; i < languages.length; i++) {
                    ...
                    const description = entity.descriptions[language];
                    wikidataDescription = description && description.value;
                    break;
                }
            }
        }
        description.value = description.source === 'local' && page.description || wikidataDescription;
 ...
 * @return {!Promise} a promise resolving as an JSON object containing the response
 */
mwapi.getPageFromMobileview = function(app, req) {
    ...
    return api.mwApiGet(req, query).then((response) => {

 ...
 * @param {!Object} coordinates the coordinates object from the MW API
 * @return {?Object} the primary Earth coordinates, if any
 */
mwapi.getPrimaryEarthCoordinates = (coordinates) => {
    if (Array.isArray(coordinates)) {
        const primary = coordinates.filter(c => c.globe === 'earth' && c.primary === '');
        if (primary.length) {
            return {
...
function getMetadataActionApi(app, req) {
    ...
    return BBPromise.props({
        ...
        const coordinates = page && page.coordinates;

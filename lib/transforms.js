...
transforms.shortenPageInternalLinks = function(doc) {
    ...
    const dbTitle = _getParsoidLinkTitle(doc);
    const prefix = './';
 ...
 */
transforms.runPcsTransforms = function(doc, restApiBaseUri, processing) {
    const dbTitle = _getParsoidLinkTitle(doc);
    transforms.preprocessParsoidHtml(doc, processing);
    pcsTransforms.runPcsTransforms(doc, restApiBaseUri, dbTitle);
};

// visible for testing
transforms._rmMwIdAttributes = _rmMwIdAttributes;
transforms._getBaseUri = _getBaseUri;
transforms._getParsoidLinkTitle = _getParsoidLinkTitle;

transforms.relocateFirstParagraph = pcsTransforms.relocateFirstParagraph;

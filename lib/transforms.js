...
const hideRedLinks = require('./transformations/legacy/hideRedLinks');
const relocateFirstParagraph = require('./transformations/legacy/relocateFirstParagraph');
...
const pcsAddSectionEditButtons
    = require('./transformations/pcs/addSectionEditButtons').addSectionEditButtons;
const pcsHead = require('./transformations/pcs/head');
const pcsHideRedLinks = require('./transformations/pcs/hideRedLinks');
const pcsAdjustThumbWidths = require('./transformations/pcs/adjustThumbWidths');
const pcsLazyLoadImagePrep = require('./transformations/pcs/lazyLoadImagePrep');
const pcsRelocateFirstParagraph = require('./transformations/pcs/relocateFirstParagraph');
const pcsPrepForTheme = require('./transformations/pcs/prepForTheme');
const pcsWidenImages = require('./transformations/pcs/widenImages');
const summarize = require('./transformations/summarize').summarize;
const stripGermanIPA = require('./transformations/stripGermanIPA');
...
function _applyOptionalParsoidSpecificTransformations(doc, references) {
    ...
    if (references) {
        _rmAttributes(doc, 'a:not([rel=nofollow],[rel~=mw:ExtLink],[rel~=mw:referencedBy])',
            ['rel']);
        _rmAttributes(doc, 'div', ['data-mw', 'typeof']);
    ...
    _rmAttributes(doc, 'b', ['about', 'data-mw', 'typeof']);
    _rmAttributes(doc, 'br', ['about', 'data-mw', 'typeof']);
    _rmAttributes(doc, 'code', ['about', 'data-mw', 'typeof']);
    _rmAttributes(doc, 'p', ['data-mw', 'typeof']);
    ...
    _rmAttributes(doc, 'link', ['data-mw', 'typeof']);
    _rmAttributes(doc, 'img', ['about', 'alt', 'resource']);
    _rmAttributes(doc, 'figure-inline', figureInlineAttributes);
    _rmAttributes(doc, 'span', figureInlineAttributes);
    _rmAttributes(doc, 'style', ['about', 'data-mw']);
    ...
    _rmMwIdAttributes(doc, 'br');
    _rmMwIdAttributes(doc, 'cite');
 ...
 */
transforms.stripUnwantedWikiContentForApp = function(doc, legacy) {
    hideRedLinks(doc);
};
...
transforms.stripWiktionarySpecificMarkup = function(doc) {
    const unwanted = [ '.ib-brac', '.ib-content', '.defdate' ];
    rmElementsWithSelector(doc, unwanted.join());
};
...
const IPA_TARGET_ATTRIBUTE = '[data-mw*="target":{"wt":"IPA"]';
const IPA_TARGET_SELECTOR = `figure-inline${IPA_TARGET_ATTRIBUTE},span${IPA_TARGET_ATTRIBUTE}`;

/**
 * Nukes stuff from the DOM we don't want from Parsoid for the summary.
 */
transforms.stripUnneededSummaryMarkup = function(doc) {
    _removeUnwantedElements(doc, [ 'span:empty' ]);
    stripGermanIPA(doc.body);
    rmElementsWithSelector(doc.body, IPA_TARGET_SELECTOR);
    rmElementsWithSelector(doc.body, 'span[style="display:none"]');
    rmElementsWithSelector(doc.body, 'span[class*=error]');
...
/**
 * Removes unwanted spans frequently found in section heading HTML.
 */
transforms.stripUnneededMetadataMarkup = function(doc) {
    _removeUnwantedElements(doc, [
 ...
 */
transforms.runPcsTransforms = function(doc, restApiBaseUri, linkTitle) {
    transforms.stripReferenceListContent(doc);
    transforms.stripUnneededMarkup(doc, false);
    pcsHideRedLinks(doc);
    pcsAdjustThumbWidths(doc);
    pcsPrepForTheme(doc);
    pcsRelocateFirstParagraph(doc);
    pcsAddSectionEditButtons(doc, linkTitle);
    pcsWidenImages(doc);
    pcsLazyLoadImagePrep(doc);
    pcsHead.addCssLinks(doc, restApiBaseUri);
    pcsHead.addMetaViewport(doc);
    pcsHead.addPageLibJs(doc);
};
...
transforms.legacy.relocateFirstParagraph = relocateFirstParagraph;

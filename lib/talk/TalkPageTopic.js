...
class TalkPageTopic {
	...
	parseTitle(chunk) {
        ...
        && chunk.tagName.match(headerTagRegex)) {
			...
			this.depth = parseInt(chunk.tagName.slice(1), 10) || 1;
			return true;
		}
		return false;
	}
	...
	pushCurrentReply() {
		...
		let chunkReplacements = this.chunksForCurrentReply.map((chunk, _index, chunks) => {
			const replacement = util.replacementFor(chunk, chunks.slice(0, _index));
		...
		let depth = -1;
		const reply = chunkReplacements.reduce((acc, chunkReplacement, _index, chunks) => {
		...
		}, '').trim().replace(newlineRegex, (match, newlines) => '<br>'.repeat(Math.min(2, newlines.length)));
		this.chunksForCurrentReply = [];

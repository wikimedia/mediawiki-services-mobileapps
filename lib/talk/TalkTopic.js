...
const arraysOfNodesAroundBreaksReducer = element => {
  let results = [];
  let currentElement = element.firstChild;
  while (currentElement) {
      const isBR = currentElement.tagName && currentElement.tagName === 'BR';
      if (isBR || !currentElement.previousSibling ) {
        results.push([]);
      }
      if (!isBR) {
        results[results.length - 1].push(currentElement);
      }
...
const breaksReducerTagsToIgnore = new Set(['PRE', 'LI', 'DT', 'DD']);
...
const soughtElementsInSectionReversed = (sectionElement, doc) => {
  return Array.from(sectionElement.querySelectorAll('p,li,dt,dd,th,td,pre,div,blockquote,center'))
    .reduce((accumulator, element) => {
      if (!breaksReducerTagsToIgnore.has(element.tagName)) {
          arraysOfNodesAroundBreaksReducer(element)

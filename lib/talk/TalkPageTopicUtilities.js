...
/**
 * @module lib/talk/TalkPageTopicUtilities
 */
...
const isList = chunk => chunk.isTag && listTags.has(chunk.tagName);

const listItemTags = new Set(['LI', 'DD', 'DT']);
const isListItem = chunk => chunk.isTag && listItemTags.has(chunk.tagName);
...
const escape = text => text.replace(escapeRegex, (match) => escapeMap[match] || '');
 ...
 */
const getUserTalkURLRegex = language => {
...
const endsWithSignature = (chunks, language) => {
	...
	for (let i = chunks.length - 1; i >= 0; i--) {
		...
		if (href && href.match(getUserTalkURLRegex(language))) {
...
const additionalLinkTextForPotentialEmptyEndAnchor = (tag, chunks) => {
	...
	const fileName = href.slice(Math.max(0, href.lastIndexOf('/') + 1));
	return `[${ fileName }]`;
};

const onlyNewlineRegex = /^\n+$/;
const isOnlyNewlines = (chunk) => !chunk.isTag && chunk.text.match(onlyNewlineRegex);

const nonListReplyDelimiters = new Set(['BR', 'P', 'TD']);
const isNonListReplyDelimiter = chunk => chunk.isTag && nonListReplyDelimiters.has(chunk.tagName);

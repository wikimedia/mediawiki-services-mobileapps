...
const depthIndicatingAncestorTags = new Set(['DL', 'UL', 'OL']);
const getReplyDepth = element => {
  let familyTreeTags = relationships.getFamilyTree(element).map(e => e.tagName);
  let depth = familyTreeTags.filter(tag => depthIndicatingAncestorTags.has(tag)).length;
...
class TalkReply {
  ...
  convertToListContainingSelfAndItems(replyArray) {
    ...
    let newText = this.isListItemOrdered ? '<ol>' : '<ul>';
    newText += `<li>${this.html}</li>`;
    replyArray.forEach(reply => {
      newText += `<li>${reply.html}</li>`;
    });
    newText += this.isListItemOrdered ? '</ol>' : '</ul>';
    this.html = newText;
  }

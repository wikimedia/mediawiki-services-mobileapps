...
const parseUserTalkPageDocIntoTopicsWithReplies = (doc, lang) => {
  ...
  const topics = flattenedSectionsInDoc(doc)
  ...
  topics.forEach((topic, i) => {
    ...
    // Since one of the sha's uses the `id` above we can't calculate them until after the fix above
    // is applied. Once T222419 is fixed and the `data-mw-section-id` fix mentioned above is applied

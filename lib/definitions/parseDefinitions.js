	...
	for ( let j = 0; j < partsOfSpeechHeadings.length; j++ ) {
		...
		if ( partsOfSpeech[ wikiLangCode ].indexOf( header ) > -1 ) {
			...
			for ( let i = 0; i < defnList.length; i++ ) {
				definitionSection.definitions.push(
					constructDefinition( defnList[ i ], wikiLangCode )
				);
			}
...
function iterateOverLanguageSections( doc, domain, title ) {
	...
	if ( wikiLangCode === 'en' ) {
		transforms.rmElementsWithSelector( doc, 'ul' ); // remove citations
		transforms.rmElementsWithSelector( doc, '.nyms' ); // remove semantic relations, see
	} // https://phabricator.wikimedia.org/T191365

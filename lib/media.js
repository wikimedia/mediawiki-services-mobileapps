...
const resolveTitleRedirects = (req, results) => {
    ...
    return api.mwApiGetBatched(req, query, titles, api.getCommonsDomain(req.params.domain))
    .then((res) => {
        res && res.forEach(redirect => {
            // check if result is a redirect
            if (!redirect.from && !redirect.to) {
                return;
            }
            for (let i = 0; i < results.length; i++) {
                if (results[i].title === redirect.from.replace(/ /g, '_')) {
                    results[i].title = redirect.to.replace(/ /g, '_');
                    break;
                }
            }
        });

...
function makeResults(pages, siteinfo) {
    return pages.filter(page => page.imageinfo).map((page) => {
        ...
        const canonicalTitle = mwapi.getDbTitle(page.title, siteinfo);
        ...
        return {
            titles: {
                ...
                display: page.pageprops && page.pageprops.displaytitle || page.title
            },
            thumbnail: {
...
function getImageInfo(app, req, si) {
    ...
    return api.mwApiQuery(app, req.params.domain, params).then((pages) => {
        return pages ? makeResults(pages, si) : [];
    });
}

function combineResults(apiResponse, pageMediaList) {

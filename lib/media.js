...
function getStructuredArtistInfo(html) {
    ...
    if (doc.body.textContent === html) {
        ...
        if (a && a.getAttribute('title') === `User:${doc.body.textContent}`
...
function makeResults(items, siteinfo) {
    return items.map((item) => {
        ...
        return {
            ...
            file_page: `https://commons.wikimedia.org/wiki/${canonicalTitle}`,
...
module.exports = {
    ...
    Audio,
    testing: {
        // todo: move processing logic in routes/media to lib/media and export stuff only

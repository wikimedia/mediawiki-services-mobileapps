...
const getStructuredSrcSet = (adjustedSrcSet) => {
    if (!adjustedSrcSet || adjustedSrcSet.length === 0) {
        return;
    }
    let allSrcSet = adjustedSrcSet && adjustedSrcSet.split(',');
    return allSrcSet && allSrcSet.map(srcset => {
...
function getMediaItemInfoFromPage(html) {
    ...
    const results = [].map.call(elems, (elem) => {
        ...
        if (mediaType === Video) {
            ...
            scaleThumb(img);
            let adjustedSrcSet = img.getAttribute('srcset');
            srcset = getStructuredSrcSet(adjustedSrcSet);
        }
        const result = {

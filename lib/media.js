...
function getAudioSources(elem) {
    return [].map.call(elem.getElementsByTagName('source'), (source) => {
        return { url: source.getAttribute('src') };
    });
}
...
function getMediaItemInfoFromPage(html) {
    ...
    const results = [].map.call(elems, (elem) => {
        ...
        let uploadUrl = resource && resource.getAttribute('src');
        let startTime;
        let endTime;
        let thumbTime;
        let audioType;
        ...
        if (mediaType === Video) {
            ...
            sources = getVideoSources(elem);
            uploadUrl = sources && sources[0] && sources[0].url;
        ...
        const result = {
            ...
            repository: uploadUrl && uploadUrl.includes('commons') ? 'commons' : 'local'
        };

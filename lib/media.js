...
const kartographerRegex = /(^|\s)mw:Extension\/mapframe\b/;
...
function getMediaType(elem) {
	...
	if (typeOf) {
		// Kartographer media can also have mw:File/Thumb value
		// defined causing them to be included as images
		if (kartographerRegex.test(typeOf)) {
			return Unknown;
		}

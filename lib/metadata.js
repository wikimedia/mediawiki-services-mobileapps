...
function buildTocEntries( sections ) {
	...
	let lastLevel = 0;
	sections
		.filter( ( s ) => s.id > 0 && ( s.toclevel && s.toclevel < TOC_DEPTH_LIMIT ) )
		.forEach( ( s ) => {
...
/**
 * Builds variant languages object for the provided metadata with
 * the content URLs for desktop and mobile.
 * @param {!Object} req the request object
 * @param {!Object} meta page metadata from the MW API
 * @param {!Object} titleObj a mediawiki-title Title object
 * @return {!Object} result the variant object response
 */
function buildVariantInfo( req, meta, titleObj ) {
	if ( !meta.variants ) {
		return;
	}
	const result = {};
	const vtitle = meta.varianttitles;
	meta.variants.forEach( ( variant ) => {
		result[ variant ] = { display_title: vtitle[ variant ] || meta.displaytitle };
	} );
	return result;
}
...
function buildMetadata( req, htmlResponse, meta, si ) {
	...
	const titleObj = mwapi.getTitleObj( req.params.title, si );
	return {
		...
		variants: buildVariantInfo( req, meta, titleObj )
	};
}

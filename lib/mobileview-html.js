...
function addParsoidHead(document, options) {
    ...
    if (!page) {
        throw new Error('Missing mobileview response for mobileview-html');
    }

    const domain = options.domain;
    if (!domain) {
        throw new Error('Missing domain for mobileview-html');
    }

    document.documentElement.setAttribute('about',
        `http://${domain}/wiki/Special:Redirect/revision/${page.revision}`);
    ...
    if (page.normalizedtitle) {
        isVersionOf.setAttribute('href', `//${domain}/wiki/${page.normalizedtitle}`);
    }
    ...
    base.setAttribute('href', `//${domain}/wiki/`);
...
function requestAndProcessPage(req, scripts) {
    return BBPromise.props({

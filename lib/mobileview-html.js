...
const addProtectionMetaTags = require('./transforms').addProtectionMetaTags;
...
function addParsoidHead(document, options) {
    ...
    isVersionOf.setAttribute('href', `//${options.domain}/wiki/${page.displaytitle}`);
...
function buildPage(req, mwResponse, meta, processingScript, options) {
    ...
    const metaObj = { parsoid: { meta: {} }, mw: meta };
    ...
    return BBPromise.props({
        doc: MobileHTML.promise(document, processingScript, options),
        meta: getMeta(req, mwResponse)
    }).then(result => {
        // addProtectionMetaTags and addPageHeader are usually called from a processing script,
        ...
        addProtectionMetaTags(result.doc, metaObj);

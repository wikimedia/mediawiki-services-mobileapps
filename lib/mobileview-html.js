...
function requestAndProcessPageIntoMobileHTML(req, scripts, baseURI) {
    return requestAndProcessPageIntoParsoid(req, scripts, baseURI).then(result => {
        const metadata = result.metadata;
        metadata.baseURI = baseURI;
        return MobileHTML.promise(result.doc, metadata).then(mobileHTML => {

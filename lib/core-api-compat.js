	...
	if (!req.params.revision) {
		...
		return followRequest(req, restReq)
			.then((response) => {
				...
				if (
					req.mwPageInfo &&
					req.mwPageInfo.lastrevid &&
					parsoidResponse.revision < req.mwPageInfo.lastrevid
				) {
					req.cachingMiddleware = {
						skipCacheSet: true
					};
					req.app.metrics.makeMetric({
						type: 'Counter',
						name: 'parsoid_output_stale_content_total',
						labels: {
							names: ['domain', 'path']
						},
						prometheus: {
							name: 'parsoid_output_stale_content_total',
							help: 'Total count of parsoid output requests that returned stale content',
							staticLabels: req.app.metrics.getServiceLabel()
						}
					}).increment(1, [req.params.domain, req.route.path]);
				}
				return parsoidResponse;
			});
	} else {

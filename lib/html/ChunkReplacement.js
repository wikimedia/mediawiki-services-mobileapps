...
/**
 * @module lib/html/ChunkReplacement
 */
...
class ChunkReplacement {
	...
	static trimWhitespace(chunkReplacements) {
		...
		chunkReplacements = chunkReplacements.filter((chunk, index) => indicies[index]);
	...
	static removeUnterminatedTags(chunkReplacements) {
		...
		return chunkReplacements.filter((chunk, index) => indicies[index]);
	}

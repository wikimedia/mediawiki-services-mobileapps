...
const reNonWhiteSpace = /\S/;

/**
 * Checks if the section is empty. Ie. no visible characters remain, ignoring white space,
 * after the heading and the reference list are removed.
 * @param {!Element} sectionElement an ancestor element of the reference list
 * @return {!boolean} true if section heading object should be included in the lists
 */
const shouldAddReferenceList = (sectionElement) => {
    return !reNonWhiteSpace.test(sectionElement.textContent);
};
...
function extractReferenceLists(document, logger) {
    ...
    refListElements.forEach((refListElement) => {
        ...
        if (shouldAddReferenceList(sectionElement)) {
            referenceLists.push({
                id: about,

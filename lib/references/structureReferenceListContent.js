...
const structureBackLinks = (listItemElement, logger) => {
    ...
    if (element.tagName === 'A') {
        ...
        logger.log('warn/ref', `unexpected child tag in back link: ${element.tagName}`);
    }
    return resultArray;
};
...
const buildOneReferenceItem = (listItemElement, logger) => {
    ...
    while (element) {
        if (element.getAttribute('rel') === 'mw:referencedBy'
            || (element.getAttribute('class') === 'mw-cite-backlink')) {
            backLinks = structureBackLinks(listItemElement, logger);
        } else if (element.tagName === 'SPAN' &&
            (element.classList.contains('mw-reference-text')
                || element.classList.contains('reference-text'))) {
            content = getReferenceContent(element);
        } else {
            logger.log('warn/ref', `unexpected child tag in ref text: ${element.outerHTML}`);
        }

...
function mergeChunkedProperties(spec, chunk) {
    ...
    Object.keys(chunk).forEach(prop => {
        ...
        if (prop === 'components') {
            spec[prop] = spec[prop] || {};
            Object.keys(chunk[prop]).forEach(childProp => {

                if (chunk[prop][childProp].constructor === Object) {
                    spec[prop][childProp] = Object.assign(
                        spec[prop][childProp] || {}, chunk[prop][childProp]);
                } else {
                    spec[prop][childProp] = chunk[prop][childProp];
                }
            });

...
function getModified(doc) {
    const meta = doc.querySelector('head > meta[property="dc:modified"]');
    if (!meta) {
        return undefined;
    }
 ...
 */
function preprocessedDocumentPromise(app, res, optimized) {
    ...
    return mUtil.createDocument(res.body)
    .then((doc) => {
        if (optimized) {
            // Note: these properties must be obtained before stripping markup
...
function pageDocumentPromise(app, req, optimized) {
    return getParsoidHtml(req)
        .then((res) => {
            return preprocessedDocumentPromise(app, res, optimized);
        });
}

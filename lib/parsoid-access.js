 ...
 */
function mobileHTMLPromiseFromHTML(app, res, html, outputMode) {
    ...
    .then((doc) => {
        return MobileHTML.promise(doc, meta, outputMode);
    });
}
...
function mobileHTMLPromise(app, req, outputMode) {
    return getParsoidHtml(req)
        .then((res) => {
            return mobileHTMLPromiseFromHTML(app, res, res.body, outputMode);
        });
}
...
function pageHtmlPromiseForReferences(app, req) {
    return getParsoidHtml(req)
        .then((response) => {
            const metadata = getRevAndTidFromEtag(response.headers);
            metadata._headers = {
                'Content-Language': response.headers && response.headers['content-language'],
                Vary: response.headers && response.headers.vary
            };

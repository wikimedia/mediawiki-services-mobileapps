...
/**
 * @module transformation/pcs/head
 */
 ...
 */
function addMetaTags(document, metaData) {
	const head = document.head;
	if (!head) {
		return;
	}
	const protections = metaData.mw.protection;
	...
	const originalimage = metaData.mw.originalimage;
	if (originalimage) {
		const meta = document.createElement('meta');
...
/**
 * Adds a meta tag listing the predicted topics for the article to the head.
 * The topics come from the Lift Wing outlink-topic-model inference service
 * and are embedded as a JSON array of {topic, score} objects:
 * <meta property="pcs:topics"
 *  content='[{"topic":"Culture.Biography.Biography*","score":0.55}]'>
 *
 * @param {Document} document DOM document
 * @param {?Array<{topic: string, score: number}>} topics predicted topics
 */
function addPredictedTopics(document, topics) {
	if (!Array.isArray(topics)) {
		return;
	}
	const meta = document.createElement('meta');
	meta.setAttribute('property', 'pcs:topics');
	/* DOM sink status: safe - content is serialized model output, not user input */
	meta.setAttribute('content', JSON.stringify(topics));

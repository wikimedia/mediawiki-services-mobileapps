...
function addCssLinks(document, baseUri) {
    ...
    if (headEl) {
        headEl.appendChild(createStylesheetLinkElement(document,
            `${BASE_URI}data/css/mobile/base`));
        headEl.appendChild(createStylesheetLinkElement(document,
            `${baseUri}data/css/mobile/site`));
    }
}
...
function createScriptElement(document) {
    ...
    el.setAttribute('src', `${BASE_URI}data/javascript/mobile/pagelib`);
    return el;
}
 ...
 * A short inline script to trigger the loading of the lazy loaded images since the content-html

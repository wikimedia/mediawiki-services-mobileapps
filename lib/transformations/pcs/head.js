...
function addCssLinks(document, options) {
    ...
    if (bodyEl) {
      bodyEl.appendChild(createStylesheetLinkElement(document,
            `${options.baseURI}data/css/mobile/pagelibpost`));
    }
}
...
/**
 * Adds the script element to html/body/
 * <script src=http://localhost:6927/meta.wikimedia.org/v1/data/javascript/mobile/pagelib></script>
 * @param {Document} document DOM document
 * @param {!object} options options object with the following properties:
 *   {!string} baseURI base URI for common links
 */
function createPostScriptElement(document, options) {
    const el = document.createElement('script');
    el.setAttribute('src', `${options.baseURI}data/javascript/mobile/pagelibpost`);
...
function addPageLibJs(document, options) {
    ...
    const bodyEl = document.querySelector('html > body');
    if (bodyEl) {
      bodyEl.appendChild(createPostScriptElement(document, options));
    }
}

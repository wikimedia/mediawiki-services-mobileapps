...
function addCssLinks(document, options) {
    ...
    if (headEl) {
        ...
        headEl.appendChild(createStylesheetLinkElement(document,
            `${options.baseURI}data/css/mobile/components`));
    }
}
 ...
 * @param {!object} options options object with the following properties:
 *   {!string} baseURI base URI for common links
 * @param {!string} endpoint which script to fetch from the API
 */
function createScriptElement(document, options, endpoint) {
    const el = document.createElement('script');
    el.setAttribute('src', `${options.baseURI}data/javascript/mobile/${endpoint}`);
    return el;
}
...
function addPageLibJs(document, options) {
    const headEl = document.head;
    if (headEl) {
        headEl.appendChild(createScriptElement(document, options, 'pcs'));
    }
    ...
    if (bodyEl) {
      ...
      bodyEl.appendChild(createScriptElement(document, options, 'vendor'));
      bodyEl.appendChild(createScriptElement(document, options, 'components'));
      bodyEl.appendChild(createScriptElement(document, options, 'webcomponents'));
    }
}

...
function createLeadSpan(doc, childNodes) {
    const leadSpan = doc.createElement('span');
    const firstGoodParagraphIndex = findFirstGoodParagraphIn(childNodes);
 ...
 * @param {!Document} doc a DOM Document for Element creation
 * @param {!Element} leadSectionEl the lead section Element
 */
function moveFirstGoodParagraphUp(doc, leadSectionEl) {
    if (!leadSectionEl) {
        return;
    }

    const childNodes = leadSectionEl.childNodes;
    if (!childNodes) {
        return;
    }

    const leadSpan = createLeadSpan(doc, childNodes);
    leadSectionEl.insertBefore(leadSpan, leadSectionEl.firstChild);
}

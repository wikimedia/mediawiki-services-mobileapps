...
const SINGLE_STRING_REGEX = /^[a-zA-Z_0-9-]{1,200}$/; // words + dashes
const HEX_REGEX = /^#(0x)?[0-9a-f]{3,8}$/i;
// eslint-disable-next-line max-len
const RGB_REGEX = /^rgba?\(\s*(\d{0,3})\.?\d*\s*[ ,]\s*(\d{1,3})\s*[ ,]\s*(\d{1,3})\s*([,/]\s*(\d{0,3}\.?\d*%?))?\s*\)$/;
...
const SANITIZE_RULES = {
    ...
    allowedStyles: {
        '*': {
            'background': [ HEX_REGEX, RGB_REGEX, SINGLE_STRING_REGEX ],
            'border': [ ANY_REGEX ],
            'color': [ HEX_REGEX, RGB_REGEX, SINGLE_STRING_REGEX ],

...
const ANY_REGEX = /^[^:]{1,80}$/; // but no : or / and not over 80 characters
const DECIMAL_REGEX = /^-?\d*\.?\d+$/;
const CSS_SIZE_REGEX = /^-?\d*\.?\d+(%|pc|pt|px|rem|em|ex|ch|vh|vw|vmin|vmax|mm|cm|in|lin)$/;
const SINGLE_STRING_REGEX = /^[a-zA-Z_0-9]{1,80}$/; // single ASCII words only, no spaces
const HEX_REGEX = /^#(0x)?[0-9a-fA-F]{3,8}$/;
const RGB_REGEX = /^rgba?\([a-zA-Z_0-9,./% -]{6,40}\)$/;
const HSL_REGEX = /^hsla?\([a-zA-Z_0-9,./% -]{6,40}\)$/;

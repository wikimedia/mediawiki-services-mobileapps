...
/**
 * @module lib/transformations/shortenPageInternalLinks
 */
...
module.exports = (doc) => {
	 ...
	 */
	const buildSelector = (_dbTitle) => {
		const start = 'a[href^=';
		const end = ']';
		const quoteIndex = _dbTitle.search(/['"]/);
		const _prefix = './';
		let selectorTitle;
		if (quoteIndex === -1) {
			// no quote found. Can use the full title.
			selectorTitle = `${ _dbTitle }#`;
		} else {
			// cannot use the full title :(
			selectorTitle = _dbTitle.slice(0, Math.max(0, quoteIndex));
		}
		return `${ start }${ _prefix }${ selectorTitle }${ end }, ${ start }${ selectorTitle }${ end }`;
	};

...
const hasOnlyWhitespace = node =>
    node.nodeType === NodeType.TEXT_NODE && REGEX_WHITESPACE.test(node.textContent);

const getCiteNoteId = (listItemElement) => listItemElement.getAttribute('id');
...
const structureBackLinks = (listItemElement, logger) => {
    ...
    if (singleBackLinksElement) {
        ...
        if (multipleBackLinksRoot) {
            ...
            logger.log('info/ref', `no back links found for ${getCiteNoteId(listItemElement)}`);
        }
    }
    return backLinks;
};
...
const getReferenceContent = (spanElement) => {
    ...
    let currentNode = spanElement.firstChild;
    while (currentNode) {
        if (!hasOnlyWhitespace(currentNode)) {
            addHtmlToContent(content, currentNode.outerHTML || _.escape(currentNode.textContent));
            if (currentNode.nodeType === NodeType.ELEMENT_NODE) {
                addCitationsToContent(content.citations, currentNode);
            }
        }

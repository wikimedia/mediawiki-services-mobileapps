 ...
 * @param {string[]} keepAttributes attributes to preserve in flattened elements
 */
function flattenElements(document, selector, keepAttributes = DEFAULT_KEEP_ATTRIBUTES) {
    const elements = document.querySelectorAll(selector);
    for (let i = 0; i < elements.length; i++) {
        const oldElement = elements[i];
        const newNode = createReplacementNode(oldElement, document, keepAttributes);

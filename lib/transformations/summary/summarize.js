...
/**
 * @module transformation/summary/summarize
 */
...
/** @type {RegExp} */
const reParenthesesMathExpressionPresent = /\(( |&nbsp;)*<i>([A-z]|[0-9+-]){1}<\/i>/;
/** @type {RegExp} */
const reBoldTextPresent = /<b>(([\w+| |']+?)|())\([a-zA-Z ']+\)<\/b>/;
...
function summarize(html) {
	return mUtil.createDocument(html)
		.then((doc) => {
			...
			rmElements(doc.body, IGNORE_NODES);
			...
			if (!reProbableFormulaPresent.test(html)
				&& !reParenthesesMathExpressionPresent.test(html)
				&& !reBoldTextPresent.test(html)) {
...
module.exports = {
	...
	testing: {
		...
		reParenthesesMathExpressionPresent
	}
};

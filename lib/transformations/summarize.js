...
const reProbableChemFormulaPresent = /<su[bp]>[0-9+-]{1,3}<\/su[bp]>/;
...
function summarize(html) {
    ...
    rmElementsWithSelector(doc.body, 'math,table,div,input,script,style,ul.gallery');
    rmElementsWithSelector(doc.body, '.mw-ref,.reference,.noexcerpt,.nomobile,.noprint,.sortkey');
    for (let i = 8, runAgain = true; i > 0 && runAgain; i--) {
        runAgain = rmElementsWithSelector(doc.body, 'span:empty,b:empty,i:empty,p:empty');
    }
    ...
    if (!reProbableChemFormulaPresent.test(html)) {
        ...
        // One difference to #1 is that in addition to a space the non-latin colon or comma could
        // also trigger the removal of parentheticals.
    ...
    // Replace any leading spaces or &nbsp; before punctuation for latin and non-latin

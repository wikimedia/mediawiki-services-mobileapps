...
/**
 * @module lib/mobile/mobile-request-util
 */
...
/**
 * Checks if the request user agent is a Wikipedia app
 *
 * @param req
 * @return {boolean}
 */
mRequestUtil.isWikipediaApp = (req) => {
	const ua = req.get('user-agent');
	return !!ua && ua.startsWith('WikipediaApp/');
};
 ...
 * @param metadata
 */
mRequestUtil.setXAnalytics = (req, res, metadata) => {
	if (!mRequestUtil.isWikipediaApp(req)) {
		return;
	}
	const parts = [];
	if (metadata.mw.pageid !== undefined) {
		parts.push(`page_id=${ metadata.mw.pageid }`);
	}

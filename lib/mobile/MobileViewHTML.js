...
/**
 * @module lib/mobile/MobileViewHTML
 */
 ...
 *
 * @param {Document} document DOM document
 * @param {!object} mobileview response body from action=parse
...
function addParsoidHead(document, mobileview, metadata) {
	...
	/* DOM sink status: safe - content from mwapi response */
	head.appendChild(createMetaElement(document, 'dc:modified', lastmodified));
...
function wrapImagesInSpanElements(document) {
	Array.from(document.querySelectorAll('a.image')).forEach((imageLink) => {
		const spanElement = document.createElement('span');
		spanElement.className = 'mw-default-size';
		/* DOM sink status: safe - content from parsoid output */
		spanElement.innerHTML = imageLink.outerHTML;
		/* DOM sink status: safe  - content from parsoid output */
		imageLink.parentNode.replaceChild(spanElement, imageLink);
	});
}
...
function convertToParsoidDocument(document, mobileview, metadata) {
	...
	wrapImagesInSpanElements(document);

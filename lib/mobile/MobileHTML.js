...
class MobileHTML extends DocumentWorker {
		...
		if (this.isRemovableElement(element, tagName, id, cls)) {
			...
			if (this.isTopLevelSection(tagName, element)) {
				...
				switch (tagName) {
					...
					case 'IMG':
						this.makeSchemeless(element, 'src');
						this.makeSchemeless(element, 'srcset');
					...
					case 'UL':
						...
						break;
					case 'LI':
						// Images within list items should not be widened
	...
	finalizeStep() {
		...
		*/
		...
		// Numerous TemplateStyles look for this class to apply styles:
		...
		// Hook into existing styles from Minerva and other MW bits
		body.classList.add('skin-minerva');

		// This is necessary for hiding elements with class=mw-empty-elt, otherwise
		// those elements will create unnecessary whitespace in the output.
		body.classList.add('mw-hide-empty-elt');

		// This is necessary for a few things, including:
		// - Making tables (infoboxes) fill the width of the parent, in our case the collapsible container.
	...
	/**
	 * Adds the article's predicted topics to the resulting document as a
	 * meta tag in the head.
	 *
	 * @param {?Array<{topic: string, score: number}>} topics predicted topics
	 *   from the outlink-topic-model inference service
	 */
	addPredictedTopics(topics) {
		this.metadata.predictedTopics = topics;
		head.addPredictedTopics(this.doc, topics);
	}
	...
	/**
	 * Prepare a reference for mobile-html reflist output. Adjusts the structure
	...
	prepareReference(reference) {
		...
		while (child) {
			...
			if (toRemove.tagName === 'OL' && toRemove.classList.contains('mw-subreference-list')) {
				subrefContainer = toRemove;
			}
		...
		const itemDiv = this.doc.createElement('div');
		itemDiv.classList.add(ReferenceCollection.CLASS.ITEM);
		...
		const footnoteNumber = reference.element.getAttribute('data-mw-footnote-number');
		const originalAnchor = this.referenceAnchors[reference.id];
		if (footnoteNumber) {
			anchor.textContent = `[${ footnoteNumber }]`;
		} else if (originalAnchor) {
		...
		if (originalAnchor) {
			...
			const unlinkedNumber = this.doc.createElement('span');
			unlinkedNumber.textContent = anchor.textContent;
			numberDiv.appendChild(unlinkedNumber);
		}

		const referenceBodyDiv = this.doc.createElement('div');
		referenceBodyDiv.classList.add(ReferenceCollection.CLASS.BODY);
		itemDiv.appendChild(referenceBodyDiv);

		if (subrefContainer) {
			containerDiv.appendChild(subrefContainer);
		}
	...
	isGalleryImage(image) {
		...
		return ((image.width >= minImageSize && image.height >= minImageSize) || isGalleryUnorderedListImage);
	}
	...
	prepareGalleryUnorderedList(element) {
		...
		if (
			...
			const galleryImgs = Array.from(element.querySelectorAll('.thumb img'));

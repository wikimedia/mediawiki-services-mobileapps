...
class MobileHTML extends DocumentWorker {
    ...
    checkElementForThemeExclusion(element) {
        const className = 'notheme';
        if (this.themeExcludedNode && !this.isSpecificHTMLStructureToSkipClassAdding(element)) {
    ...
    /**
     * Determines if an element is of specific structure
     * when 'notheme' class adding is to be skipped.
     *
     * @param {Element} element
     * @return {boolean} true if the element is of specific structure
     * when 'notheme' must not be added to an element.
     */
    isSpecificHTMLStructureToSkipClassAdding(element) {
        return (
            // T264687 - prevent applying 'notheme' class name
            // for all td-s because we need to style them depending the theme

 ...
 */
  ...
  prepareReference(reference) {
    ...
    const Link = (props) => {
      const attributes = {
        [ReferenceCollection.BACK_LINK_ATTRIBUTE]: JSON.stringify(props.reference.backLinks)
      };
      let text = '↑';
      if (props.original) {
        const group = props.original.getAttribute('data-mw-group');
        if (group && constants.refGroupsGeneratedByCSS.has(group)) {
          attributes['data-mw-group'] = group;
          attributes.style = props.original.getAttribute('style');
          text = '';
        } else if (props.original.textContent) {
          text = Reference.truncateLinkText(props.original.textContent);
        }
      }

      return html`
        <a id="back_link_${props.reference.id}"
           class=${ReferenceCollection.CLASS.BACK_LINK_ANCHOR}
           href="./${props.metadata.linkTitle}${ReferenceCollection.BACK_LINK_FRAGMENT_PREFIX}${props.reference.id}"
           ...${attributes}
           >
          ${text}
        </a>
    ...
    const ReferenceEntry = (props) => html`
      <div class=${ReferenceCollection.CLASS.REF}>
        <div class=${ReferenceCollection.CLASS.BACK_LINK_CONTAINER}>
          <${Link} reference=${props.reference} original=${props.originalAnchor} metadata=${props.metadata} />
        </div>
        <div class=${ReferenceCollection.CLASS.BODY}>
          ${raw(reference.textElement.outerHTML)}
        </div>
      </div>
    `;
    ...
    reference.element.innerHTML = html`
      <${ReferenceEntry} reference=${reference}
                         originalAnchor=${this.referenceAnchors[reference.id]}
  ...
  prepareSectionHeader(header, section, sectionId, isReferenceSection, doc) {
    ...
    if (!isReferenceSection) {
      ...
      const sectionContents = doc.createElement('div');
      while (el) {
          const toRemove = el;
          ...
          sectionContents.appendChild(toRemove);
      }
      ...
      section.innerHTML = html`
        <wmui-accordion header=${header.textContent} section-id=${sectionId} link-title=${this.metadata.linkTitle}>
          ${raw(sectionContents.innerHTML)}
        </wmui-accordion>
      `;
    }
  }

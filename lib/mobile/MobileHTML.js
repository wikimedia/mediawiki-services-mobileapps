...
class MobileHTML extends DocumentWorker {
    ...
    prepareElement(element) {
        ...
        if (this.isRemovableElement(element, tagName, id, cls)) {
            ...
            if (this.isTopLevelSection(tagName, element)) {
                ...
                /*
                T261019 -Set specific 'no-lazy-load' class
                for images inside the zero and first section
                */
                if (this.currentSectionId === '0' || this.currentSectionId === '1') {
                    this.performNoLazyLoadImg(element);
                }
    ...
    performNoLazyLoadImg(element) {
        ...
        if (Object.keys(tables).length > 0) {
            Object.values(tables).forEach(table => {
                /*
                All tables except 'infobox' (collapsible)
                should not have lazy-loaded images
                */
                if (table.className && !table.className.includes('infobox')) {
                    LazyLoad.addImageNoLazyLoadClass(element);
                }
            });
        } else {
            // Remove lazy load for all images in particular section
            LazyLoad.addImageNoLazyLoadClass(element);
        }
    }
    ...
    checkElementForThemeExclusion(element) {
        const noThemeClass = 'notheme';

        const style = element.getAttribute('style');
        const inlineStylesCondition = style && constants.inlineBackgroundStyleRegex.test(style);

        if (
            inlineStylesCondition && this.excludeFigcaptionElems(element) ||
            inlineStylesCondition && this.excludeEquationBoxElems(element) ||
            inlineStylesCondition && this.excludeThSpanElem(element) ||
            inlineStylesCondition && this.excludeTdElem(element)
        ) {
            return;
        } else if (
            this.excludeFigcaptionElems(element) ||
            this.excludeEquationBoxElems(element) ||
            this.excludeThSpanElem(element) ||
            this.excludeTdElem(element)
        ) {
            return;
        } else if (inlineStylesCondition) {
     ...
     * @return {boolean}
     */
    excludeThSpanElem(element) {
        return (
            element.localName === 'span' &&
            element.parentNode.localName === 'th' &&
            element.parentNode.getAttribute('class') === 'notheme'
        );
    }
     ...
     * @return {boolean}
     */
    excludeFigcaptionElems(element) {
     ...
     * @return {boolean}
     */
    excludeEquationBoxElems(element) {
    ...
    moveLeadParagraphUp(sectionId, section) {
        ...
        const tagName = child && child.tagName;
        // T277465 - Some templates might have a style tag
        // that doesn't have a hatnote class as first element. Skip it.
        if (tagName && (tagName === 'STYLE' || tagName === 'LINK')) {
    ...
    isRemovableSpan(span, classList) {
        if (!span.firstChild) {
            // T202923 - trace missing anchor span tags
            if (classList && classList.includes('anchor')) {
                return false;
            }
            return true;
        }
    ...
    prepareImage(image) {
        if (this.isGalleryImage(image)) {
            ...
            if (!image.classList.contains(LazyLoad.CLASSES.NO_LAZY_LOAD)) {
    ...
    prepareSupLinks(supTag) {
        const supTouchStyle = 'margin: -4px -16px -16px -16px; padding: 4px 16px 16px 16px;',
            supTouchLeftAreaStyle = 'margin: -4px 0 -16px -16px; padding: 4px 2px 16px 16px;',
            supTouchRightAreaStyle = 'margin: -4px -16px -16px 0; padding: 4px 16px 16px 2px;',
            supTouchMiddleStyle = 'padding: 4px 2px 16px 2px;',
            supTouchOverlapStyle = 'margin: -4px 0 -16px 0; padding: 4px 2px 16px 4px;';

        if (supTag.firstElementChild && supTag.firstElementChild.localName === 'a') {
            supTag.firstElementChild.classList.add('reference-link');
        }

        const setSupChildInlineStyle = (element, inlineStyle) => {
            if (element.firstElementChild && element.firstElementChild.localName === 'a') {
                if (!element.firstElementChild.getAttribute('style')) {
                    element.firstElementChild.setAttribute('style', inlineStyle);
                } else {
                    const supStyle = element.firstElementChild.getAttribute('style');
                    const supStyLeTouchArea = supStyle.concat(' ', inlineStyle);
                    element.firstElementChild.setAttribute('style', supStyLeTouchArea);
                }
            }
        };
        ...
        const applySupRTL = () => {
            if (supSiblingPrevCondition && supSiblingNextCondition) {
                setSupChildInlineStyle(supTag, supTouchMiddleStyle);
            } else if (supSiblingPrevCondition) {
                if (supAnchorNextCondition || supTextNextCondition) {
                    setSupChildInlineStyle(supTag, supTouchOverlapStyle);
                } else {
                    setSupChildInlineStyle(supTag, supTouchLeftAreaStyle);
                }
            } else if (supSiblingNextCondition) {
                if (supAnchorPrevCondition || supTextPrevCondition) {
                    setSupChildInlineStyle(supTag, supTouchOverlapStyle);
                } else {
                    setSupChildInlineStyle(supTag, supTouchRightAreaStyle);
                }
            } else {
                if (supAnchorNextCondition || supTextNextCondition) {
                    setSupChildInlineStyle(supTag, supTouchLeftAreaStyle);
                    ...
                    setSupChildInlineStyle(supTag, supTouchStyle);
                }
            }
        };

...
mUtil.setContentType = function(res, spec) {
    ...
    res.type(`application/json; charset=utf-8; profile="https://www.mediawiki.org/wiki/Specs/${spec.name}/${spec.version}"`);
};
...
mUtil.mergeByProp = function(arr1, arr2, prop, pushIfKeyNotFound) {
    ...
    arr2.forEach(function(arr2obj) {
        const arr1obj = arr1.find(function(arr1obj) {
...
mUtil.getRbPageSummaryUrl = function(restbase_tpl, domain, title) {
    const request = restbase_tpl.expand({
        request: {
            params: {
                domain: domain,

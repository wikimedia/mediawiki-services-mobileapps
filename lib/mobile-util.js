...
mUtil.CONTENT_TYPES = {
    ...
    mediaList: { name: 'MediaList', version: '1.0.1', type: 'application/json' }
};
...
mUtil.createDocument = (html) => {
    if (!html || typeof html[0] !== 'string') {
        return P.reject(new HTTPError({
                status: 500,
                type: 'invalid_content_type',
                title: 'Only text/html is allowed',
                detail: 'Set your request Content-Type header to text/html'
        }));
    }

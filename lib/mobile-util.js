...
mUtil.CONTENT_TYPES = {
    html: { name: 'HTML', version: '1.7.0', type: 'text/html' },
    mobileSections: { name: 'mobile-sections', version: '0.14.2', type: 'application/json' },
    media: { name: 'Media', version: '1.4.0', type: 'application/json' },
    ...
    summary: { name: 'Summary', version: '1.3.7', type: 'application/json' },
...
mUtil.setLanguageHeaders = (response, headers) => {
    if (headers) {
        ...
        if (lowerHeaders.vary) {
            ...
            response.vary(lowerHeaders.vary);
        }

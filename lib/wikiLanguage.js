...
let languageMappingCache = null;

if (!languageMappingCache) {
    api.getWikiLanguageData().then(wikiLanguages => {
        return ( languageMappingCache = wikiLanguages );
    });
}
...
function getAllLanguageVariantsForLanguageCode(code) {
    ...
    const mapping = languageMappingCache[code];
    const codes = [code];
    if (!mapping) {
        return codes;
    }
...
function isLanguageCodeWithVariants(code) {
    ...
    return languageMappingCache[code] !== undefined;
}
...
function relevantWikiLanguageCodesFromAcceptLanguageHeader(value, wikiCode) {
    const sortedCodes = parseAcceptLanguageHeaderIntoLanguageTags(value).sort((a, b) => {
        ...
        // Handle 'ike' prefix of 'iu' language code
        if (normalizedCode === 'ike') {
            codeMapping = languageMappingCache.iu;
        } else {
            codeMapping = languageMappingCache[normalizedCode];
        }
    ...
    if (wikiCode) {
        let filteredLanguages = sortedCodes.filter((code) => {
            return code.startsWith(wikiCode);
        });
        if (!filteredLanguages.includes(wikiCode)) {
            filteredLanguages.push(wikiCode);
        }
        // Handle 'iu' language code
        if (wikiCode === 'iu') {
            filteredLanguages = _.uniq(sortedCodes.concat(filteredLanguages));
        }

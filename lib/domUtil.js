...
domUtil.getBaseUri = (doc) => {
    const base = doc.querySelector('html > head > base');
...
domUtil.getParsoidPlainTitle = (doc) => {
    const title = doc.querySelector('html > head > title');
...
domUtil.getParsoidLinkTitle = (doc) => {
    const link = doc.querySelector('html > head > link[rel="dc:isVersionOf"]');
    if (!link) {
        return undefined;
    }
